...
class QLRunner(Runner):
    def __init__(self, agent, env, train, load_path, save_path, moving_average_window_size):
        super(QLRunner, self).__init__(agent, env, train, load_path, save_path)
    ...
    def summarize(self):
        ...
        self.writer.add_summary(tf.compat.v1.Summary(
            value=[tf.compat.v1.Summary.Value(tag='Epsilon per Episode', simple_value=self.agent.eps)]),
            self.episode
        )
