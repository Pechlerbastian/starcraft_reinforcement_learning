...
class Network:
    def __init__(self, alpha=0.0007, c_H=0.005, c_val=0.5):
        ...
        self.hidden_layer = Dense(units=128, activation='relu')(self.flatten)
        self.output_actor = Dense(units=8, activation='softmax')(self.hidden_layer)
    ...
    def save_model(self, filename):
        self.model.save(filename + "/AC2Network.h5")

    def load_model(self, filename):
        self.model = load_model(filename + "/AC2Network.h5")
...
class A2CAgent(AbstractAgent):
    def __init__(self, screen_size, alpha=0.0007, gamma=0.99, c_val=0.5, c_H=0.005, nWorkers=8, train=True,
        ...
        if train:
            ...
            print(f"1 new worker initialized!")
        ...
        for i in range(self.nWorkers):
            if self._MOVE_SCREEN.id in self.sorted_obs[i].observation.available_actions:
                ...
                if marine is None:
                    ...
                    self.worker_pipes[i].send(["action", self._dir_to_sc2_action(actions[i], marine_coords)])
            else:
    ...
    def step(self, observation):        # parameter 'observation' only needed for overwriting of abstract method
        ...
        old_actions = [np.random.choice(list(self.network.get_directions().keys()), size=1, p=prob)[0]
                       for prob in action_probabilities]

        # 3. execute actions and receive new observations
        new_sorted_obs = self.execute_actions(old_actions)
        ...
        if self.train:
            ...
            for i in range(self.nWorkers):
                ...
                if not self.sorted_obs[i].last():
                    ...
                    action_idx = list(self._DIRECTIONS.keys()).index(old_actions[i])
                    self.data[i].append_transition(state=old_states[i], action=action_idx, reward=new_rewards[i],
                                                   done=new_done[i] or new_sorted_obs[i].reward == 1,
                                                   critic=critic_predictions[i])

            # 6. check for possible network update
        ...
        # 7. | 5. update observation
        self.sorted_obs = new_sorted_obs

        # 8. | 6. return score if any worker finished its episode, None otherwise
        if True in new_done:
            return self.score
        else:
            return None
