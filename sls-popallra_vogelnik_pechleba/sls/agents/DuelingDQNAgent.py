...
class DuelingDQNAgent(AbstractAgent):
        ...
        if self._MOVE_SCREEN.id in obs.observation.available_actions:
            ...
            if self.train:
                ...
                self.update_experience_replay(
                    Transition(self.s_old, self.a_old, r_new, s_new, obs.last() or r_new == 1))
    ...
    def save_model(self, filename):
        self.model.save(filename + "/DuelingDQNetwork.h5")

    def load_model(self, filename):
        self.model = load_model(filename + "/DuelingDQNetwork.h5", custom_objects={'lmd': self.lmd})
    ...
    def generate_mini_batch(self):
        return [self.experience_replay[index] for index in
                np.random.choice(range(0, self.experience_replay_length), 32)]

    def gradient_descent(self, batch):
        # create training data
        x_train = np.asarray([b.s_old for b in batch])
        y_train = []
        y = self.model.predict_on_batch(x_train)
        y_t = self.target_model.predict_on_batch(np.asarray([b.s_new for b in batch]))

        for trans, predicted_by_target, predicted_by_model in zip(batch, y_t, y):
            # get index of used action in s_old
            idx = list(self._DIRECTIONS.keys()).index(trans.a_old)

            # set value of target output for the specific action used in s_old according to pseudocode
            if trans.done:
                new_value = trans.r_new
            else:
                # get prediction of model in order to create error = 0 for all actions not taken in s_old
                new_value = trans.r_new + self.gamma * np.amax(predicted_by_target)

            # collect predictions as training target data
            y_target = np.asarray([value for value in predicted_by_model])
            y_target[idx] = new_value
